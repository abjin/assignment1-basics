...
def run_multihead_self_attention_with_rope(
    ...
    mha = model.MultiheadSelfAttention(d_model, num_heads, rope=rope)
    mha.load_state_dict(
        {
            "q_proj.weight": q_proj_weight,
            "k_proj.weight": k_proj_weight,
...
def run_transformer_block(
    ...
    """
    rope = model.RotaryPositionalEmbedding(theta, d_model // num_heads, max_seq_len)
    block = model.TransformerBlock(d_model, num_heads, d_ff, rope=rope)
...
def run_get_batch(
    ...
    """
    return nn_utils.get_batch(dataset, batch_size, context_length, device)
...
def run_save_checkpoint(
    ...
    """
    nn_utils.save_checkpoint(model, optimizer, iteration, out)
...
def run_load_checkpoint(
    ...
    """
    return nn_utils.load_checkpoint(src, model, optimizer)
